...
class UniHavenTests(TestCase):
    ...
    def test_accommodation_lookup_no_suggestions(self, mock_get):
        # Test the case where no suggestions are returned
        ...
        with patch("builtins.print"):  
            with patch("api.models.lookup_coordinates_and_geoaddress") as mock_lookup:
                ...
                accommodation = Accommodation.objects.create(
                    title="No Geo Apartment",
                    description="A nice place",
                    property_type="AP",
                    price=1000.00,
                    beds=2,
                    bedrooms=1,
                    address="Unknown St, HK",
                    flat_number="3C",
                    floor_number="3",
                    available_from=date.today(),
                    available_to=date.today() + timedelta(days=30),
    ...
    def test_accommodation_lookup_exception(self, mock_get):
        # Test the case where an exception occurs during lookup
        mock_get.side_effect = requests.RequestException("Network error")
        with patch("builtins.print"):  
            with patch("api.models.lookup_coordinates_and_geoaddress") as mock_lookup:
                mock_lookup.return_value = (None, None, "")
                accommodation = Accommodation.objects.create(
                    title="Error Apartment",
                    description="A nice place",
                    property_type="AP",
                    price=1000.00,
                    beds=2,
    ...
    def test_reservation_str(self):
        # Test the __str__ method of Reservation
    ...
    def test_user_str_with_full_name(self):
        # Test __str__ when user has a full name
    ...
    def test_user_staff_profile_auto_creation(self):
        # Test automatic creation of StaffProfile
        user = User(
            username="staff2",
            university="HKU",
    ...
    def test_user_admin_profile_role(self):
        # Test get_user_role returning 'admin'
        user = User.objects.create_user(
            username="admin2",
            password="testpass123",
            university="HKU",
    ...
    def test_user_unknown_role(self):
        # Test get_user_role returning 'unknown'
        user = User.objects.create_user(
            username="unknown1",
            password="testpass123",
            university="HKU",
    ...
    def test_user_clean_multiple_roles(self):
        # Test clean method raising ValidationError for multiple roles
        user = User.objects.create_user(
            username="multi_role",
            password="testpass123",
            university="HKU",
